...
def _send_notification_notify2(title: str, message: str, icon_path: str | None = None) -> bool:
    """Sends a notification using the notify2 library if available."""
    if not NOTIFY2_AVAILABLE:
        logger.debug("notify2 library not available, cannot send notification via notify2.")
        return False
    ...
    try:
        ...
        icon = icon_path if icon_path and Path(icon_path).exists() else "dialog-information"
...
def _send_notification_dbus(title: str, message: str, icon_name: str = "dialog-information") -> bool:
    """Sends a notification using raw dbus-python (fallback if notify2 fails/missing)."""
    if not DBUS_PYTHON_AVAILABLE:
        logger.warning("dbus-python library not available. Cannot send notification via dbus.")
        return False
    ...
    try:
        ...
        notify_interface.Notify(
            ...
            dbus.Int32(5000)                # expire_timeout in milliseconds (0 = default, -1 = never)
        )
...
def send_notification(title: str, message: str):
    ...
    logger.warning("Failed to send notification using all available methods (notify2/dbus-python).")
