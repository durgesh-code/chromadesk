...
try:
    ...
    try:
        ...
        logger.critical(f"Failed to import core modules. Make sure script is run correctly: {e}")
        sys.exit(1)
...
def send_notification(title: str, message: str, urgency: str = "normal"):
    ...
    logger.debug(f"Attempting to send notification: Title='{title}', Urgency='{urgency}'")
    try:
        ...
        if not subprocess.check_output(['which', 'notify-send']).strip():
             logger.warning("'notify-send' command not found. Cannot send desktop notifications.")
             return

        # Check for necessary env var (informational)
        if "DBUS_SESSION_BUS_ADDRESS" not in os.environ:
             logger.warning("DBUS_SESSION_BUS_ADDRESS not found in environment. Notifications might fail from systemd.")

        cmd = ['notify-send', '-a', 'ChromaDesk', '-u', urgency, title, message]
        subprocess.run(cmd, check=True, timeout=5)
        logger.debug("Notification command executed.")
    except FileNotFoundError:
         logger.warning("'notify-send' command not found (checked again). Cannot send desktop notifications.")
...
def run_daily_update():
    ...
    try:
        ...
        logger.info(f"Config loaded: enabled={is_enabled}, region={region}, keep={keep_history_count}, last_update={last_update_str}")
    except Exception as e:
        logger.critical(f"Failed to load configuration: {e}. Aborting.", exc_info=True)
        # Send critical notification if possible
        send_notification("ChromaDesk Error", "Failed to load configuration. Daily update aborted.", "critical")
        return False
    ...
    if last_update_str == today_str:
        ...
        return True
         ...
         logger.info(f"Last update was {last_update_str}, proceeding with update for {today_str}.")
    ...
    if not core_history.ensure_wallpaper_dir():
        ...
        send_notification("ChromaDesk Error", "Cannot create or access wallpaper directory. Update failed.", "critical")
        return False
    ...
    if not bing_info or not bing_info.get('full_url'):
        ...
        send_notification("ChromaDesk Update Failed", f"Could not fetch Bing image information for region {region}.", "normal")
    ...
    try:
        target_filename = core_history.get_bing_filename(bing_info['date'], bing_info['full_url'])
        local_path = wallpaper_dir / target_filename
    except Exception as e:
         logger.error(f"Failed to generate filename for Bing image: {e}", exc_info=True)
         send_notification("ChromaDesk Error", "Internal error generating filename.", "critical")
         return False
    ...
    if local_path.is_file():
        ...
        download_success = core_downloader.download_image(bing_info['full_url'], local_path)
        if not download_success:
            logger.error(f"Failed to download Bing image.")
            send_notification("ChromaDesk Update Failed", f"Could not download the Bing image.", "normal")
    ...
    if not set_success:
        ...
        send_notification("ChromaDesk Update Failed", f"Could not set the downloaded wallpaper: {local_path.name}", "critical")
        # Don't mark as updated if setting failed
        return False


    # 9. Update State in Config
    logger.info(f"Wallpaper set successfully. Updating last_update_date to {today_str}.")
    try:
